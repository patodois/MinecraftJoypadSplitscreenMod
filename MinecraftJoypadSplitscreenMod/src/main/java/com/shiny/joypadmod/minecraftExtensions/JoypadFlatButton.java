...
/** PixelLab-textured widget that scales with Minecraft's GUI scale. */
public class JoypadFlatButton extends GuiButton {
    private GlyphArt.Icon icon;
    private boolean selected;
    public JoypadFlatButton selected(boolean value) { selected=value; return this; }
    ...
    @Override public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        ...
        JoypadTheme.plate(xPosition,yPosition,width,height,hover||selected,enabled||selected);
        if(icon!=null) {
        ...
        drawCenteredString(mc.fontRendererObj, mc.fontRendererObj.trimStringToWidth(displayString, width - 4),
            xPosition + width / 2, yPosition + (height - 8) / 2, !enabled && !selected ? 0x8A8D77 : hover || selected ? 0xF2EAA9 : PixelTheme.TEXT);
    }
}
